	...
	namespace Tiled
	{
		...
		std::map<const std::string, std::string> GetProperties(lua_State*L)
		{
			...
			if (!lua_istable(L, -1))
			{
				lua_pop(L, 1);
				// STL: table1 --
				throw(LuaError("Table has not 'properties' key."));
			}
			// STK: table1 -- table2
			lua_insert(L, 1);
			// STK: table2 table1 --
			...
			while(lua_next(L, 1))
			{
				...
				if (!lua_isstring(L, -2))
					throw(LuaError("A key in properties is a non string value."));
				else if(!lua_isstring(L, -1))
					throw(LuaError("A key in properties returned a non string value."));
				// STK: table2 table1 -- string string
